...
const newDevice = (): Device => {
  return {
    deviceName: faker.company.name(),
    deviceType: faker.helpers.arrayElement(['Smartphone', 'Tablet', 'Laptop', 'Desktop', 'IoT Sensor', 'Router', 'Switch']),
    location: faker.number.int(100),
    connections: faker.number.int(1000),
    batteryLevel: faker.number.int(100),
    status: faker.helpers.shuffle<Device['status']>([
      'online',
      'offline',
      'maintenance',
    ])[0]!,
...
export function makeData(...lens: number[]) {
  const makeDataLevel = (depth = 0): Device[] => {
    ...
    return range(len).map((_d): Device => {
      return {
        ...newDevice(),
  ...
  if (columnFilters && columnFilters.length > 0) {
    filteredData = filteredData.filter(row => {
      return columnFilters.every(filter => {
        ...
        const rowValue = row[id as keyof Device]
  ...
  if (sorting && sorting.length > 0) {
    filteredData.sort((a, b) => {
      for (const sort of sorting) {
        const { id, desc } = sort
        const aValue = a[id as keyof Device]
        const bValue = b[id as keyof Device]
